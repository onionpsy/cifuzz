...
func (b *Builder) BuildAndCreateCoverageReport(fuzzTest string, outputPath string) (string, error) {
	var err error
	...
	if strings.HasSuffix(fuzzTest, "_bin") {
		trimmedLabel := strings.TrimSuffix(fuzzTest, "_bin")
		cmd := exec.Command("bazel", "query", trimmedLabel)
		err = cmd.Run()
		if err == nil {
			fuzzTest = trimmedLabel
		}
	}
